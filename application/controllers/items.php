...
class Items extends Secure_area implements iData_controller
{
	...
	function index($limit_from=0)
	{
		...
		$data['manage_table']=get_items_manage_table( $items, $this );
	...
	function get_sizes_by_category() {
		...
		foreach($this->Item_sizes->get_sizes_by_category_id($this->input->post('category_id')) as $row) {
			$item_sizes[$row['size_id']] = $row['name'];
		}
...
/**END GARRISON ADDED**/
	...
	function view($item_id=-1)
	{
		...
		$data['selected_supplier_id'] = $this->Item->get_info($item_id)->supplier_id;
	...
	function inventory($item_id=-1)
	{
        ...
        foreach($stock_locations as $location_detail)
        {
        	...
        	foreach($item_units as $unit_detail)
        	{
        		...
        		$data['item_units'][$unit_id] = $unit_detail['unit_name']; 
	...
	function save($item_id=-1)
	{
		...
		'size_id'=>$this->input->post('size_id')=='' ? null:$this->input->post('size_id'),
	...
	{	
        ...
        $unit_id = $this->input->post('trans_unit');
		$inv_data = array
		(
			'trans_date'=>date('Y-m-d H:i:s'),
			'trans_items'=>$item_id,
			...
			'trans_inventory'=>$this->input->post('trans_inventory'),
			'trans_unit'=>$unit_id
		);
		...
		$item_quantity= $this->Item_quantities->get_item_quantity($item_id,$location_id,$unit_id);
		$item_quantity_data = array(
		'quantity'=>$item_quantity->quantity + $this->input->post('trans_inventory'),
		);
		if($this->Item_quantities->save($item_quantity_data,$item_id,$location_id,$unit_id))
		{			
	...
	function bulk_update()
	{
		...
		foreach($_POST as $key=>$value)
		{
			...
			elseif($value!='' and !(in_array($key, array('submit', 'item_ids', 'tax_names', 'tax_percents', 'category_id'))))
			{
				$item_data["$key"]=$value;
			}
		}
        ...
        else
		{
            if (($handle = fopen($_FILES['file_path']['tmp_name'], "r")) !== FALSE)
            {
                ...
                while (($data = fgetcsv($handle)) !== FALSE)
                {
                    ...
                    if ($category != "")
                    {
                    	$result = $this->Item_category->get_category_by_name($category);
                    	if ($result->num_rows() > 0)
                    	{
	                    	$item_data['category_id'] = $result->row()->category_id;
                    	}
                    	else 
                    	{
                    		$item_category_data = array('name' => $category);
							$this->Item_category->save($item_category_data);
							$item_data['category_id'] = $item_category_data['category_id'];
                    	}
                    }
                    ...
                    if(!$invalidated && $this->Item->save($item_data)) 
                    {
                        ...
                        $allowed_units = $this->Item_units->get_undeleted_all();
                        for ($col = 24; $col < $cols; $col = $col + 2)
                        {
                            $location_id = $data[$col];
                            $unit_id = $data[$col+1];
                            if (array_key_exists($location_id, $allowed_locations) && array_key_exists($unit_id, $allowed_units))
                            {
                                $item_quantity_data = array (
                                    'item_id' => $item_data['item_id'],
                                    'location_id' => $location_id,
                                ...
                                $this->Item_quantities->save($item_quantity_data, $item_data['item_id'], $location_id, $unit_id);
